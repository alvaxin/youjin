...
function publicRoom(room, token) { return { code:room.code, host:room.host, mySeat:room.members.find(m=>m.token===token)?.seat, members:room.members.map(({name,seat})=>({name,seat})), status:room.status, hasState:!!room.state }; }
...
const server=http.createServer(async(req,res)=>{
  ...
  if (url.pathname==='/api/rooms' && req.method==='POST') { const body=await read(req), account=user(body.token); if(!account)return json(res,401,{error:'账号失效'}); const room={code:code(),host:account.token,members:[{token:account.token,name:account.name,seat:0}],listeners:new Map(),state:null,status:'lobby'}; rooms.set(room.code,room); return json(res,200,publicRoom(room,account.token)); }
  const match=url.pathname.match(/^\/api\/rooms\/([A-F0-9]{6})(?:\/(join|events|state|action))?$/); if(match){ const [,roomCode,operation]=match; const room=rooms.get(roomCode); if(!room)return json(res,404,{error:'房间不存在'}); if(operation==='events'){ const token=url.searchParams.get('token'); if(!roomFor(roomCode,token))return json(res,403,{error:'无权访问'}); res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive'}); send(res,'room',publicRoom(room,token)); if(room.state)send(res,'state',stateFor(room,token)); room.listeners.set(token,res); req.on('close',()=>room.listeners.delete(token)); return; }
    ...
    if(!roomFor(roomCode,body.token))return json(res,403,{error:'无权访问'}); if(operation==='state'){ if(room.host!==body.token)return json(res,403,{error:'只有房主可同步牌局'}); room.state=body.state; if(room.status==='lobby')room.status='playing'; pushRoom(room); pushState(room); return json(res,200,{ok:true}); } if(operation==='action'){ const member=room.members.find(item=>item.token===body.token); if(['start','reset'].includes(body.action)&&room.host!==body.token)return json(res,403,{error:'只有房主可执行此操作'}); if(body.action==='start'){ room.status='playing'; pushRoom(room); } push(room,'action',{from:body.token,seat:member.seat,action:body.action,payload:body.payload||{}}); return json(res,200,{ok:true,room:publicRoom(room,body.token)}); }
  }
  const file=url.pathname==='/'?'index.html':url.pathname.slice(1); const target=path.join(root,file); if(!target.startsWith(root)||!fs.existsSync(target))return json(res,404,{error:'not found'}); res.writeHead(200,{'Content-Type':mime[path.extname(target)]||'application/octet-stream'}); fs.createReadStream(target).pipe(res);
});
