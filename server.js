...
const server=http.createServer(async(req,res)=>{
  ...
  const match=url.pathname.match(/^\/api\/rooms\/([A-F0-9]{6})(?:\/(join|events|state|action|actions))?$/); if(match){ const [,roomCode,operation]=match; const room=rooms.get(roomCode); if(!room)return json(res,404,{error:'房间不存在'}); if(operation==='events'){ const token=url.searchParams.get('token'); if(!roomFor(roomCode,token))return json(res,403,{error:'无权访问'}); res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-cache','Connection':'keep-alive','X-Accel-Buffering':'no'}); res.write(': connected\n\n'); send(res,'room',publicRoom(room,token)); if(room.state)send(res,'state',stateFor(room,token)); room.listeners.set(token,res); req.on('close',()=>room.listeners.delete(token)); return; }
    ...
    if(!roomFor(roomCode,body.token))return json(res,403,{error:'无权访问'}); if(operation==='state'){ if(room.host!==body.token)return json(res,403,{error:'只有房主可同步牌局'}); room.state=body.state; if(room.status==='lobby')room.status='playing'; pushRoom(room); pushState(room); return json(res,200,{ok:true}); }
    if(operation==='actions'){ if(room.host!==body.token)return json(res,403,{error:'只有房主可读取操作队列'}); const actions=room.actions.splice(0); return json(res,200,{actions}); }
    if(operation==='action'){ const member=room.members.find(item=>item.token===body.token); if(['start','reset'].includes(body.action)&&room.host!==body.token)return json(res,403,{error:'只有房主可执行此操作'}); if(body.action==='start'){ room.status='playing'; pushRoom(room); } const action={id:room.nextActionId++,from:body.token,seat:member.seat,action:body.action,payload:body.payload||{}}; if(room.host!==body.token)room.actions.push(action); push(room,'action',action); return json(res,200,{ok:true,room:publicRoom(room,body.token)}); }
  }
